...
object Anagrams {
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    val raw = occurrences match {
      case Nil => List(List())
      case (ch, ct) :: remain => (0 to ct).toList.flatMap(i => {
        combinations(remain).map(combination => (ch, i) :: combination)
      })
    }
    raw.map(list => list.filter(elt => elt._2 > 0))
  }
